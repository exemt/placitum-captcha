/*
 * Скрипт страницы капчи: собирает отпечаток среды, ведёт виджет (картинку
 * или внешний) и отправляет форму. Ничего не подписывает и cookie не
 * ставит -- ключ на сервере, а клиренс появляется только в Set-Cookie
 * ответа на POST.
 *
 * Отпечаток -- хеш, не сырой вектор: на сервер уезжают 32 hex-знака. Сырые
 * строки renderer и пиксели холста не покидают браузер.
 */
(function () {
  'use strict';
  ...
  // Скрипт провайдера грузится динамически: не загрузился за timeout или
  // упал -- показываем следующий виджет. Токен виджета уходит в answer.
  var EXT = {
    turnstile: {
  ...
  function loadExternal(widget, kind, d) {
    ...
    s.onerror = fail;
    s.onload = function () {
      if (kind === 'smartcaptcha') return; // позовёт __wafSmart
      var tries = 0;
      (function poll() {
  ...
  // Следующий виджет цепочки, когда текущий не смог.
  function showFallback() {
    var list = [].slice.call(widgets);
    var i = list.indexOf(primary);
    if (i < 0 || i + 1 >= list.length) {
      var text = primary.querySelector('.ext-wait');
      if (text) text.textContent = 'Проверка недоступна. Вернитесь на сайт и попробуйте позже.';
      return;
    }
    primary.hidden = true;
    primary = list[i + 1];
