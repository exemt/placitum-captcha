/*
 * Маршрутизация страницы виджета и всё, что не является проверкой ответа.
 *
 * Адрес страницы задаёт оператор в профиле, поэтому маршрут вычисляется по
 * снимку профилей, а не зашит. Под адресом живут:
 *
 *   GET  <path>          страница с виджетом (нужен билет waf_cap)
 *   POST <path>          ответ формы
 *   GET  <path>/api      задание машинно, для SPA
 *   POST <path>/verify   ответ машинно, для SPA
 *   GET  <path>/status   есть ли клиренс у предъявителя
 *   GET  <path>/c.js     скрипт виджета и сбора отпечатка
 *   GET  <path>/healthz  проба контейнера
 */
...
/*
 * alive -- жив ли клиренс по активному списку профиля. Механизм один: кука и
 * список, и HTTP проверяет его той же меркой, что инспектор. Свежий клиренс
 * проходит по одной подписи -- запись ещё едет через секвенсор; дальше нет
 * записи -- нет клиренса, а невидимый список гасит, а не пропускает.
 */
...
// widgetView -- один провайдер на странице. Data -- JSON параметров задания
// для скрипта; у внешних это sitekey.
type widgetView struct {
	Kind    string
	Data    template.JS
	NeedJS  bool
	Primary bool
	// Image -- data:URI картинки: она обязана работать без JS, поэтому
	// едет атрибутом, а не через скрипт.
	Image template.URL
	Audio bool
}
...
/*
 * route находит профиль по адресу страницы. Хвост после адреса -- действие.
 * Профиль по адресу -- любой с этим path: он нужен только чтобы нарисовать
 * страницу "нет билета"; всё остальное берётся из профиля, названного в
 * билете.
 */
...
/*
 * session открывает билет и сверяет его с клиентом. Профиль берётся из
 * билета: страницу могли открыть по общему адресу, а политика -- у того
 * маршрута, который увёл.
 */
...
// backTo -- куда вернуть: из билета, иначе из query, иначе на корень.
...
// lang -- первый язык из Accept-Language, который есть в профиле.
...
// localPath -- та же паранойя, что в инспекторе: только локальный путь.
