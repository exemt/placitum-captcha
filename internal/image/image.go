/*
 * Картинка: текст из алфавита профиля, растр без внешних шрифтов, волновое
 * искажение и шум. Единственный провайдер, которому не нужен JS.
 *
 * Шрифт -- битовая карта 7x13 из golang.org/x/image, увеличенная в несколько
 * раз: образ без файлов шрифтов и без cgo. Читаемость человеком сохраняется
 * волной малой амплитуды; распознавание OCR затрудняется волной, линиями и
 * точками. Это не криптография: картинка отсекает дешёвые автоматы, а не
 * фермы распознавания -- те отсекает цена и лимиты.
 */
...
// Text -- случайная строка из алфавита.
...
// Render рисует текст в PNG. seed -- для детерминированной волны внутри
// одной сессии не нужен: картинка генерируется при каждом показе.
func Render(text string) []byte {
	w := padX*2 + glyphW*len(text)
	h := padY*2 + glyphH

	// Сначала текст в чёрно-белую маску в натуральном размере.
	...
	// Волна: по x и по y разной частоты и фазы.
...
// noise -- несколько линий и точки: мешают сегментации, не человеку.
