/*
 * Поколение профилей калитки из JetStream KV.
 *
 * Манифест несёт текст profile.yaml и users.yaml -- ровно те файлы, что лежат
 * в каталоге профилей. Поэтому здесь нет ни разбора профиля, ни валидации:
 * файлы раскладываются на диск, и дальше их читает тот же загрузчик, что читает
...
func Parse(raw []byte) (*Manifest, error) {
	...
	for name, profile := range m.Profiles {
		...
		for _, file := range profile.Files {
			 ...
			 * доверии к контроллеру: "../" в имени -- это запись мимо каталога
			 * поколения.
			 */
