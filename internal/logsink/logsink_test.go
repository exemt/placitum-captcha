...
func TestBatchEnvelope(t *testing.T) {
	s := New("waf-captcha-1", "captcha", nil)
	defer s.Close()
	...
	body, err := json.Marshal(batch{
		...
		Kind:   Kind,
		Writer: s.writer,
		Lines:  []Line{{Service: "captcha", Severity: "info", Text: "hello"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.V != 1 || got.Kind != "log" || got.Writer != "waf-captcha-1" {
		t.Fatalf("конверт: %s", body)
	}

	if len(got.Lines) != 1 || got.Lines[0].Text != "hello" || got.Lines[0].Service != "captcha" {
