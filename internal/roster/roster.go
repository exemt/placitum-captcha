/*
 * Серверная сторона билета: одноразовость, отзыв, счётчики, сессии виджета.
 *
 * Клиренс запечатан, и проверка базы не требует. Roster нужен ровно для
 * того, чего печать не умеет:
 *
 *   nonce   -- HTTP принимает каждый билет один раз;
 *   отзыв   -- клиренс обязан гаситься раньше срока по jti и по отпечатку;
 *   попытки -- исчерпавший попытки упирается в бан подсети;
 *   счётчики -- лимиты выдачи и проверки на подсеть, клиренсов на отпечаток;
 *   сессии  -- задание виджета (ответ картинки) под nonce.
 *
 * Отзыв инспектор читает из памяти, а не из Redis: множество ограничено сверху
 * сроком жизни сессий, а поход в хранилище на каждом запросе не оставил бы от
 * бюджета волны ничего. Цена -- окно refresh между выходом и тем, как его
 * увидят все реплики.
 */
...
// ErrNotFound -- сессии виджета под этим ключом нет: истекла или не было.
var ErrNotFound = errors.New("roster: not found")

type Roster interface {
	// Revoked -- из памяти, без сети. Горячий путь инспектора.
	Revoked(sid string) bool
	// Refresh перечитывает множество отзыва целиком.
	...
	// Put/Get/Del -- сессия виджета под ключом. Get не сжигает: одноразовость
	// обеспечивает Burn по nonce, а сессию можно читать дважды (страница и
	// её перерисовка) до первой проверки.
...
// Watch крутит обновление множества отзыва. Ошибка не гасит цикл: устаревшее
// множество лучше пустого, а совсем без отзыва калитка всё равно работает.
...
/*
 * Refresh забирает множество целиком и заодно подчищает истёкшие записи.
 * Чистка идемпотентна, поэтому её делает любая реплика: договариваться о том,
 * кто именно уберёт мусор, дороже самой уборки.
 */
...
/*
 * Revoke держит sid до истечения самого токена и ни секундой дольше: после
 * этого срока подпись всё равно не примут, а запись только занимала бы место
 * в множестве, которое читают все реплики.
 */
 ...
 * для тестов; при scale > 1 nonce, попытки и отзыв разъезжаются между
 * репликами, и это не деталь реализации, а свойство режима.
 */
type Memory struct {
	mu      sync.Mutex
