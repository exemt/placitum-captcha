...
**Инспектор без буфера не стартует.** Не видя куки пропуска, он перенаправлял бы на виджет даже тех,
кто его уже прошёл.
...
| `WAF_CAPTCHA_DATA` | пусто; в образе `/var/lib/waf/captcha` | куда сохраняется применённое поколение |
| `WAF_CAPTCHA_LOG` | `info` | уровень журнала |
| `WAF_CAPTCHA_STORE_TIMEOUT` | `150ms` | предел похода в Redis |
...
| `WAF_CAPTCHA_GEO_ADDR` | пусто | справочник сетей; пусто — бакеты по подсети и системе молчат |
| `WAF_CAPTCHA_GEO_TIMEOUT`, `WAF_CAPTCHA_GEO_NEG_MAX` | `500ms`, `0` | ожидание справочника сетей и отрицательный кэш |
...
- **Бакеты — во внутреннем Redis, не в буфере.** Перепутанные адреса дают счёт, который у
