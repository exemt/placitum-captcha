...
**Инспектор без обменника не стартует.** Не видя куки клиренса, он уводил бы на виджет даже тех,
кто его уже прошёл.
...
- **`WAF_CAPTCHA_COOKIE_SECURE=off` при TLS** — кука клиренса может уйти по открытому каналу.
